...
def fun_kmc_fa(filename, k, dirpath, newfilename, tmpdir):
    
    print(time.asctime(time.localtime(time.time())))
    print("[KMC]:   running k>", k, "\n")
    file=newfilename.split('/')[-1]
    newfile=os.path.join(dirpath, file+".txt")
    
    
    kmcStd=check_output(["./kmc", "-k{}".format(k), "-fm", "@{}".format(filename), newfilename, tmpdir], stderr=STDOUT, universal_newlines=True)
    print(kmcStd)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--k", "-k", default=21, type=int, help="kmer length")
        ...
        try:
            ...
            if type == "fasta" or type == "fa":
                for key, value in filenameDict.items():
                    fun_kmc_fa(value,k, args.dirpath, key, args.dirpath)
